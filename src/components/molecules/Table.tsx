...
function Table<T>({
  ...
  const list = isArrayData ? data : data.data || [];
  const totalData = isArrayData ? data.length : data.total || 0;

  const totalPages = Math.ceil(totalData / limit);
  const currentPageNumber = Math.floor(currentOffset / limit) + 1;

  const currentData = isArrayData
    ? list.slice(currentOffset, currentOffset + limit)
    : list;
                  ...
                  <tr>
                    {
                        columns.map((col) => {
                              const isActiveSort =
                  orderBy === (col.sortKey || col.accessor);
                            return (
                                <th
                                key={col.header}
                                scope="col"
                  ...
                  {data.data.length === 0 ? (
                    ...
                    data.data.map((item, index) => (
                      <tr key={index} className="hover:bg-slate-900/60 transition-colors">
                        {
                            columns.map((col) => (
                                <td key={col.header} className="px-6 py-4 font-medium text-slate-200">
